...
class Messages extends Admin_Controller {
    ...
    function __construct()
    {
        ...
        // Page Title
        $this->set_title( lang('menu_messages') );
    }
   ...
   /**
    ...
    {
        // check access
        $this->acl->check_access('messages', 'p_view');
    ...
    */
    public function ajax_list()
    {
        ...
        foreach ($result as $val) 
        {
            ...
            $row[]          = $val->m_favorite ? '<i class="fas fa-heart text-red"></i>' : '<i class="far fa-heart"></i>';
            $row[]          = date('g:iA j/m/y ', strtotime($val->added));
            $row[]          = action_buttons('messages', $val->id, mb_substr($val->message, 0, 50, 'utf-8'), lang('menu_message'), $val);
            $data[]         = $row;
        }
    ...
    */
    ...
    {   
        // check access
        $this->acl->check_access('messages', 'p_delete', TRUE);
        ...
        if(empty($result))
        {
            ...
            $error = sprintf(lang('alert_not_found'), lang('menu_message'));
